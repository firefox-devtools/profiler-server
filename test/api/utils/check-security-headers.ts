...
export function checkSecurityHeaders(request: Test) {
  ...
  return request
    .expect(
      ...
      `default-src 'none';base-uri 'none';font-src 'self' https: data:;form-action 'self';frame-ancestors 'none';img-src 'self' data:;object-src 'none';script-src 'self';script-src-attr 'none';style-src 'self' https: 'unsafe-inline';upgrade-insecure-requests;report-uri /__cspreport__;report-to cspreport`
    )
    .expect(
      'Report-To',
