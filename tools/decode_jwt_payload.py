...
"""Output the profile token extracted from the JWT token returned by the server.
...
def handle_input_line(line):
    ...
    This will first split the token in its 3 components, base64 decode the 2nd one
    that's the payload, json-parse it, and finally print the key "profileToken"
    from that JSON payload.
    """
