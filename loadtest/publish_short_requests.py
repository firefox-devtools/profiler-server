...
This load test will generate some loads against the publishing endpoint of the
profiler server. It uses small workloads so that the requests are faster and
less limited by the available bandwidth.
"""
import json
import os
import base64
...
@scenario(2)
async def publish_10k(session):
    await publish(session=session, data_size=10)


@scenario(1)
async def publish_50k(session):
    await publish(session=session, data_size=50)
